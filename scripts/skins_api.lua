...
AddComponentPostInit("skinner", function(self)
	...
	function self:OnLoad(data, ...)
		...
		if SKINNABLE_CHARACTERS[self.inst.prefab] and data.skin_name ~= nil then
			...
			if self.onsetskinfn then
				self.onsetskinfn(self.inst)
			end
		end
	end
	self.SetOnSetSkinFn = function(self, fn)	-- Patched by donmor: add callbacks
		self.onsetskinfn = fn
	end
end)

AddComponentPostInit("wardrobe", function(self)	-- Patched by donmor: add callbacks
	local pApplySkins = self.ApplySkins
	self.ApplySkins = function(self, doer, diff)
		pApplySkins(self, doer, diff)
		if doer.components.skinner ~= nil then
			if diff.base ~= nil then
				if Prefabs[diff.base] ~= nil then
					doer.components.skinner.onsetskinfn(doer)
				end
			end
		end	
	end
...
function _G.SpawnNewPlayerOnServerFromSim(player_guid, skin_base, clothing_body, clothing_hand, clothing_legs, clothing_feet, starting_item_skins, ...)
	...
	if player ~= nil and SKINNABLE_CHARACTERS[player.prefab] then
		...
		player:DoTaskInTime(0, function()
			...
			if skinner.onsetskinfn then	-- Patched by donmor: add callbacks
				skinner.onsetskinfn(player)
			end
		end)
	end
end
